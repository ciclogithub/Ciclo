    ...
    public class ContadoresDB
    {
        public Contadores Listar(int organizador)
        {
            try
            {
                ...
                Query query = session.CreateQuery("select (select COUNT(i.idinscricao)  from inscricoes i left join Cursos c on c.idcurso = i.idcurso where i.idstatus = 0 and c.idorganizador = @organizador) as inscricoes, (select count(*) from Instrutores where idorganizador = @organizador) instrutor, (select count(*) from cursos where idorganizador = @organizador) cursos, (select count(*) from alunos where idorganizador = @organizador) alunos, ISNULL((select(SUM((cast(cv.nrnota1 + cv.nrnota2 + cv.nrnota3 + cv.nrnota4 + cv.nrnota5 as float) / 5)) / count(*)) from cursos c inner join cursos_alunos ca on ca.idcurso = c.idcurso inner join cursos_avaliacoes cv on cv.idcursoaluno = ca.idcursoaluno where c.idorganizador = @organizador),0) avaliacaoes, (select count(*) from empresas where idorganizador = @organizador) empresas, (select count(*) from alunos a where a.idorganizador = @organizador and isnull((case when A.txcpf = '' then (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where EXISTS(select ae.idaluno from Alunos_Emails as ae WHERE ae.txemail = e.email and ae.idaluno = a.idaluno)) else (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where e.cpf_cnpj = replace(replace(a.txcpf,'.',''),'-','')) end), 0) = 1) alunos_ciclo, (select count(*) from alunos a where a.idorganizador = @organizador and isnull((case when A.txcpf = '' then(select max(e.venda) from diagweb.diagweb.ExisteCliente as e where EXISTS(select ae.idaluno from Alunos_Emails as ae WHERE ae.txemail = e.email and ae.idaluno = a.idaluno)) else (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where e.cpf_cnpj = replace(replace(a.txcpf,'.',''),'-','')) end), 0) = 2) alunos_compra, (select count(*) from empresas em where em.idorganizador = @organizador and isnull((case when em.txcnpj = '' then (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where EXISTS(select ee.idempresa from Empresas_Emails as ee WHERE ee.txemail = e.email and ee.idempresa = em.idempresa)) else (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where e.cpf_cnpj = replace(replace(em.txcnpj,'.',''),'-','')) end), 0) = 1) empresas_ciclo, (select count(*) from empresas em where em.idorganizador = @organizador and isnull((case when em.txcnpj = '' then(select max(e.venda) from diagweb.diagweb.ExisteCliente as e where EXISTS(select ee.idempresa from Empresas_Emails as ee WHERE ee.txemail = e.email and ee.idempresa = em.idempresa)) else (select max(e.venda) from diagweb.diagweb.ExisteCliente as e where e.cpf_cnpj = replace(replace(em.txcnpj,'.',''),'-','')) end), 0) = 2) empresas_compra, (select count(*) from alunos where idorganizador = @organizador and idaluno not in (select idaluno from alunos_emails) and txcpf = '') as alunos_email, (select count(*) from empresas where idorganizador = @organizador and idempresa not in (select idempresa from empresas_emails) and txcnpj = '') as empresas_email, (select count(*) from Temas where idorganizador = @organizador) temas, (select count(*) from Locais where idorganizador = @organizador) locais, (select COUNT(*) from Cursos_Alunos ca inner join Cursos c on c.idcurso = ca.idcurso where c.idorganizador = @organizador and ca.txavaliacao <> '') avalenviada, (select COUNT(*) from Cursos_Avaliacoes ca inner join Cursos_Alunos cl on cl.idcursoaluno = ca.idcursoaluno inner join Cursos c on c.idcurso = cl.idcurso where c.idorganizador = @organizador and cl.txavaliacao <> '') avalrecebido");
