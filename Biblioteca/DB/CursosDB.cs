    ...
    public class CursosDB
    {
        ...
        public List<Cursos> Listar(string curso, int page = 1, int regs = 10)
        {
            try
            {
                ...
                while (reader.Read())
                {
                    list_cursos.Add(new Cursos(Convert.ToInt32(reader["idcurso"]), Convert.ToString(reader["txcurso"]), Convert.ToInt32(reader["idtema"]), Convert.ToInt32(reader["idcategoria"]), Convert.ToInt32(reader["idlocal"]), Convert.ToString(reader["txlocal"]), Convert.ToString(reader["nrminimo"]), Convert.ToString(reader["nrmaximo"]), Convert.ToString(reader["txcargahoraria"]), Convert.ToString(reader["txdescritivo"]), Convert.ToBoolean(reader["flgratuito"]), Convert.ToString(reader["txfoto"]), Convert.ToInt32(reader["idcor"]), Convert.ToString(reader["txidentificador"]), Convert.ToInt32(reader["total"]), Convert.ToInt32(reader["idorganizador"])));
                }
                reader.Close();
                session.Close();
        ...
        public List<Mercados> ListarMercados(int id = 0)
        {
            try
            {
                ...
                DBSession session = new DBSession();
                Query query = session.CreateQuery("select m.idmercado, m.txmercado from Cursos_Mercados am left join mercados m on m.idmercado = am.idmercado where am.idcurso = @id order by m.txmercado");
                query.SetParameter("id", id);
                IDataReader reader = query.ExecuteQuery();
        ...
        public List<Especialidades> ListarEspecialidades(int id = 0)
        {
            try
            {
                ...
                DBSession session = new DBSession();
                Query query = session.CreateQuery("select e.idespecialidade, e.txespecialidade from Cursos_Especialidades ce left join especialidades e on e.idespecialidade = ce.idespecialidade where ce.idcurso = @id order by e.txespecialidade");
                query.SetParameter("id", id);
                IDataReader reader = query.ExecuteQuery();
